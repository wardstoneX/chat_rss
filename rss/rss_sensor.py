...
class RssStateInfo(object):
    ...
    def __init__(self, rss_state, ego_dynamics_on_route, world_model):
        ...
        if bool(self.is_dangerous) == True:
            print("dangerous!")
        ...
        #print("Distance to other vehicle:", self.distance)
        #print("Lateral margin:", self.lateral_margin)
        #print("Longitudinal margin:", self.longitudinal_margin)
...
class RssSensor(object):
    ...
    def destroy(self):
        ...
        # Assuming self.sensor_data is a list, you can convert it to a pretty-printed JSON string
            #print(json.dumps(self.sensor_data, indent=4))  # Pretty print with indentation

            #sys.stdout.flush()

            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            print(timestamp)
            file_path = f"sensor_data_{timestamp}.json"
            
            # Write the formatted JSON data to the file
            with open(file_path, 'w') as json_file:
                json.dump(self.sensor_data, json_file, indent=4)  # Writing the data as a formatted JSON file
    ...
    def get_steering_ranges(self):
        ...
        return ranges
    @staticmethod
    def calculate_risk(current_distance, safe_distance):
        try:
            # Ensure both inputs are treated as floats
            current_distance = float(current_distance)
            safe_distance = float(safe_distance)
        except (ValueError, TypeError) as e:
            #print(f"Error: Unable to convert values to float. current_distance: {current_distance}, safe_distance: {safe_distance}")
            return 0  # Return 0 if there's an error converting the values

        # Check if safe_distance is zero
        if safe_distance == 0:
            #print("Warning: Safe distance is zero, cannot calculate risk.")
            return 0  # No risk can be calculated if there's no safe distance

        # Check if current_distance is zero
        if current_distance == 0:
            #print("Warning: Current distance is zero, no fault since there's no current distance.")
            return 0  # No fault since there's no current distance

        # Calculate and return risk
        risk = 1 - (current_distance / safe_distance)
        # If risk is negative, return 0
        if risk < 0:
            risk = 0
    ...
    def _on_rss_response(self, response):
        ...
        if delta_time > -0.05:
            ...
            #print(self.proper_response)
            #print(", ".join(
             #       f"Object ID: {rss_state.objectId}, Situation Type: {str(rss_state.situationType).split('::')[-1]}"
             #       for rss_state in self.rss_state_snapshot.individualResponses
             #    ))
            ...
            if len(new_states) > 0:
                ...
                new_states.sort(key=lambda rss_state: rss_state.rss_state.objectId)
                #print(1)
                
            call_data = []
            for rss_state in new_states:
                situation_type = str(rss_state.rss_state.situationType).split('::')[-1]

                longitudinal_safe_distance = rss_state.rss_state.longitudinalState.rssStateInformation.safeDistance
                current_longitudinal_distance = rss_state.rss_state.longitudinalState.rssStateInformation.currentDistance

                lateral_right_safe_distance = rss_state.rss_state.lateralStateRight.rssStateInformation.safeDistance
                current_lateral_right_distance = rss_state.rss_state.lateralStateRight.rssStateInformation.currentDistance

                lateral_left_safe_distance = rss_state.rss_state.lateralStateLeft.rssStateInformation.safeDistance
                current_lateral_left_distance = rss_state.rss_state.lateralStateLeft.rssStateInformation.currentDistance

                object_data = {
                    "objectId": int(rss_state.rss_state.objectId),
                    "situation_type": situation_type,
                    "distance": float(rss_state.distance),
                    "longitudinal_margin": float(rss_state.longitudinal_margin),
                    "lateral_margin": float(rss_state.lateral_margin),
                    "is_dangerous": bool(rss_state.is_dangerous),
                    "longitudinal_safe_distance": float(longitudinal_safe_distance),
                    "current_longitudinal_distance": float(current_longitudinal_distance),
                    "lateral_right_safe_distance": float(lateral_right_safe_distance),
                    "current_lateral_right_distance": float(current_lateral_right_distance),
                    "lateral_left_safe_distance": float(lateral_left_safe_distance),
                    "current_lateral_left_distance": float(current_lateral_left_distance)
                }

            
                call_data.append(object_data)

            self.sensor_data.append(call_data)    
