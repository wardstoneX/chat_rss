...
class RssStateVisualizer(object):
    ...
    def tick(self, individual_rss_states):
        ...
        object_ids = [state.rss_state.objectId for state in individual_rss_states]
        if object_ids:  # Check if the list is not empty
            print(", ".join(map(str, object_ids)))
        ...
        for state in individual_rss_states:
            ...
            v_offset += 14
             # Render additional details below each state
            longitudinal_safe_distance = state.rss_state.longitudinalState.rssStateInformation.safeDistance
            current_longitudinal_distance = state.rss_state.longitudinalState.rssStateInformation.currentDistance

            lateral_right_safe_distance = state.rss_state.lateralStateRight.rssStateInformation.safeDistance
            current_lateral_right_distance = state.rss_state.lateralStateRight.rssStateInformation.currentDistance

            lateral_left_safe_distance = state.rss_state.lateralStateLeft.rssStateInformation.safeDistance
            current_lateral_left_distance = state.rss_state.lateralStateLeft.rssStateInformation.currentDistance

            details = [
                f"- Long. Safe: {longitudinal_safe_distance}m",
                f"- Long. Curr: {current_longitudinal_distance}m",
                f"- LatR Safe: {lateral_right_safe_distance}m",
                f"- LatR Curr: {current_lateral_right_distance}m",
                f"- LatL Safe: {lateral_left_safe_distance}m",
                f"- LatL Curr: {current_lateral_left_distance}m"
            ]

            for detail in details:
                detail_surface = self._font.render(detail, True, (255, 255, 255))
                state_surface.blit(detail_surface, (10, v_offset))
                v_offset += 14
            #print(lateral_left_safe_distance)    
